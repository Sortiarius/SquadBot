...
export default {
    ...
    execute: async function(message: Discord.Message, args: any){
        ...
        const bestMatch = stringSimilarity.findBestMatch(towerName, Object.keys(Towers)).bestMatch;
        if(bestMatch.rating <= 0.3){ return; }
        const tower: Tower = Towers[bestMatch.target];

        const Embed: Discord.MessageEmbed = new Discord.MessageEmbed()
            .setColor("#0099ff")
            .setAuthor("Tower Search", "https://cdn.discordapp.com/icons/543294798430339102/3fa3c167af9a9f40a1789c4dea3165d8.png", "")

        if(!tower.damage_type){
            Embed.addField(`**${tower.name}**`, `*${tower.armor}*`);
        } else { Embed.addField(`**${tower.name}**`, `*${tower.damage_type}, ${tower.armor}*`); }
        ...
        if(tower.damage_base && tower.damage_rand){
            ...
            Embed.addField(
                "**Damage**",
                `${tower.damage_base.toString()}}`,
                true
            );
        } else { Embed.addField('**Damage**', '*None*', true); }
