...
const cmdFs = fs.readdirSync(path.join(__dirname, './command')).filter(f => f.endsWith('.js') || f.endsWith('.ts'));
...
client.on('message', async message => {
    ...
    if(message.content.split(/ +/)[0] === `<@!${client.user!.id}>`){
        ...
        if(args[1] != "prefix") { return; }
        let prefix = args[2];
        if(prefix === undefined){ await message.channel.send("Invalid Prefix!"); }
        //console.log(prefix);
